...
preprocess() {
  ...
  do
    echo "$post"
    cat "$post" | sed -E 's/\!\[ig\]\((.*)\)/{% include amp-instagram.html shortcode="\1" %}/g' | sed -E 's/\!\[fbv\]\((.*)\)/{% include amp-fb-video-embed.html url="\1" %}/g' | sed -E 's/\!\[fbc\]\((.*)\)/{% include amp-fb-comment-embed.html url="\1" %}/g' | sed -E 's/\!\[bo\]\((.*)\)/{% include amp-beopinion.html id="\1" %}/g' | sed -E 's/\!\[fbp\]\((.*)\)/{% include amp-fb-post-embed.html url="\1" %}/g' | sed -E 's/\!\[(.*)\]\[(.*)\]\[(.*)\]\((.*)\)\((.*)\)/{% include fig.html alt="\1" desc="\2" credit-name="\3" src="\4" credit-src="\5" slug="\1" %} /g' | sed -E 's/\!\[(.*)\]\[(.*)\]\((.*)\)/{% include fig.html alt="\1" desc="\2" src="\3" slug="\1" %} /g' | sed -E 's/\!\[(.*)\]\((.*)\)/{% include fig.html alt="\1" src="\2" slug="\1" %} /g' | cat
  done
}
